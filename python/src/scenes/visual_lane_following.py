...
class VLF(BaseScene):
    def __init__(self, memory_name, camera_info, msg_queue):
        ...
        self.debug_dir = os.path.join(os.getcwd(), "capture", "vlf")
    ...
    def _draw_path(self):
        ...
        for idx in range(1, len(points)):
            ...
            color = (30, 130, 255) if self.records[-500:][idx - 1]["ok"] else (80, 80, 80)
        ...
        cv2.putText(canvas, "VLF predicted path", (24, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (20, 20, 20), 2)
        return canvas

    def _save_records(self):
        csv_path = os.path.join(self.debug_dir, "vlf_records.csv")
        with open(csv_path, "w", encoding="utf-8") as f:
            f.write("index,time,ok,mode,error,pid,target_x\n")
            for record in self.records[-1000:]:
                f.write(
                    "{index},{time:.3f},{ok},{mode},{error:.3f},{pid:.6f},{target_x}\n".format(**record)
                )
    ...
    def loop(self):
        ...
        try:
            while True:
                ...
                log.info(
                    'vlf: ok=%s mode=%s error=%.1f pid=%.3f',
                    result['ok'], result['mode'], result['error'], result['pid']
                )
                ...
                log.info(f'vlf cost {time.time() - start}')
